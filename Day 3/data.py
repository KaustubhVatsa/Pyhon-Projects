...
ATTACKS = {name: Attack(name, data['type'], data['base_damage'], data['accuracy'], data['debuff']) 
...
for name, stats in RAW_POKEMON.items():
    attack_objs = [ATTACKS[name] for name in stats['attacks']]
...
if __name__ == "__main__":  # Runs only if file executed directly (not imported).
    print(get_pokemon_names()[:3])  # First 3 names.
    pikachu = POKEMON_DATA['Pikachu']
    print(pikachu)  # Uses __str__.
    print(pikachu.attack[0])  # First attack.